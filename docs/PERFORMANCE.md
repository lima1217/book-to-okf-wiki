...
Tokens entering context to answer **one** targeted question. book-to-okf-wiki loads a
resident core (~4K) plus one compiled chapter (~1K) ≈ **5,000 tokens**.

| Book (chapter size) | Context-dump | Discovery loop | book-to-okf-wiki | vs dump / loop |
|---------------------|-------------:|---------------:|--------------:|:--------------:|
...
```bash
python3 tools/discovery_tax.py --full-text /tmp/book_okf_wiki_work/full_text.txt --target-chapter 5
```
