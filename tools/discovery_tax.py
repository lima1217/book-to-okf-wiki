...
into an agent's context to answer one targeted question:
  ...
  3. book-to-okf-wiki — a small resident OKF wiki navigation core
                      (index.md + AGENTS.md) + one compiled chapter note.
...
Usage:
  python3 tools/discovery_tax.py --full-text <full_text.txt> \
      [--package-dir <okf_wiki_folder>] [--target-chapter N] [--core-tokens 4000]
"""
...
def main() -> int:
    ...
    ap.add_argument("--package-dir", help="generated OKF wiki package folder")
    ap.add_argument("--skill-dir", help=argparse.SUPPRESS)
    ...
    ap.add_argument("--core-tokens", type=int, default=4000,
                    help="resident navigation core size if --package-dir not given (design cap)")
    args = ap.parse_args()
    ...
    # book-to-okf-wiki resident cost
    package_dir = args.package_dir or args.skill_dir
    if package_dir:
        sd = Path(package_dir)
        core_files = [sd / "index.md", sd / "AGENTS.md"]
        existing_core = [p for p in core_files if p.exists()]
        core = sum(count_tokens(p.read_text(encoding="utf-8")) for p in existing_core) if existing_core else args.core_tokens
        ...
        core_label = "measured index.md + AGENTS.md" if existing_core else "design cap"
    else:
        core = args.core_tokens
        comp_chapter = 1000
        core_label = "design cap (no --package-dir)"
    ...
    print(f"    book-to-okf-wiki     : {skill:>9,}   core [{core_label}] ({core:,}) + compiled chapter ({comp_chapter:,})\n")

    print("  book-to-okf-wiki advantage:")
